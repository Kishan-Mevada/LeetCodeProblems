 ...
 *
 *	-- Time : O(3^(m + n), where m,n are the length of word1 and word2, 3 for we have 3 choices
 *  -- Space : O(3^(m + n), each recursive call we have 3 choices
 *
 *  Note : this solution may lead to time limit exceed
 */
public class EditDistance {
	...
	private static int minDistance(int i, int j, String word1, String word2) {
		if (i == word1.length() - 1 && j == word2.length() - 1) return 0;
		if (i == word1.length() - 1) return word2.length() - j; // we have only insertion
		if (j == word2.length() - 1) return word1.length() - i; // we have only deletion

		if(word1.charAt(i) == word2.charAt(j)){
			return minDistance(i+1,j+1,word1,word2);
		}

		return Math.min(minDistance(i+1,j,word1,word2), Math.min(minDistance(i,j+1,word1,word2),minDistance(i+1,j+1,word1,word2))) + 1;
	}
